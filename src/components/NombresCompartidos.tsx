...
export default function NombresCompartidos({
          ...
          <div className="border-t border-gray-200 bg-gray-50 px-4 py-4">
            <div className="space-y-3">
              {familia.nombres.map((nombre, idx) =>
                renderNombreCompartido(
                  nombre,
                  `${familiaKey}-nombre-${idx}`,
                ),
              )}
            </div>
          </div>
      ...
      <div key={generoKey} className="mb-4 rounded-lg border border-gray-200 bg-white">
        ...
        >
          <div className="flex-1">
            ...
            <p className="mt-1 text-xs text-gray-500">
              {genero.familia} • {genero.orden} • {genero.nombres.length}{" "}
              nombres compartidos
            </p>
          </div>
          {isOpen ? (
  ...
  return (
    <div className="space-y-8">
      {/* Nombres compartidos por familia */}
      {nombresPorFamilia.length > 0 && (
        <div>
          <h3 className="mb-4 text-xl font-bold text-gray-800">
            Nombres Compartidos por Familia
          </h3>
          <p className="mb-6 text-sm text-gray-600">
            Nombres comunes que son utilizados por múltiples especies dentro de
            la misma familia taxonómica.
          </p>
          <div className="space-y-3">
            {nombresPorFamilia.map((familia) => renderFamilia(familia))}
          </div>
        </div>
      )}

      {/* Nombres compartidos por género */}
      {nombresPorGenero.length > 0 && (
        <div>
          <h3 className="mb-4 text-xl font-bold text-gray-800">
            Nombres Compartidos por Género
          </h3>
          <p className="mb-6 text-sm text-gray-600">
            Nombres comunes que son utilizados por múltiples especies dentro del
            mismo género taxonómico.
          </p>
          <div className="space-y-3">
            {nombresPorGenero.map((genero) => renderGenero(genero))}
          </div>
        </div>
      )}

      {nombresPorFamilia.length === 0 && nombresPorGenero.length === 0 && (
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-8 text-center">
          <p className="text-gray-500">
            No se encontraron nombres comunes compartidos.
          </p>
        </div>
      )}
    </div>
