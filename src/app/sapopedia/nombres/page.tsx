...
export default async function NombresPage() {
  ...
  const totalNombres = ordenes.reduce(
    (sum, orden) =>
      ...
      (orden.children?.reduce(
        (s, familia) =>
          s +
          (familia.children?.reduce(
            (g, genero) => g + genero.nombres.length,
            0,
          ) || 0),
        0,
      ) || 0),
  ...
  const totalFamilias = ordenes.reduce(
    (sum, orden) => sum + (orden.children?.length || 0),
    0,
  );
  const totalGeneros = ordenes.reduce(
    (sum, orden) =>
      sum +
      (orden.children?.reduce(
        (s, familia) => s + (familia.children?.length || 0),
        0,
      ) || 0),
    0,
  );
    ...
    <main className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8 text-center">
        ...
        <p className="text-muted-foreground mx-auto max-w-3xl text-lg">
          Nombres comunes de anfibios de Ecuador organizados por orden, familia
          y género.
        </p>
      </div>
      ...
      <div className="mb-8">
        <h2 className="mb-6 text-2xl font-bold">
          Nombres por Categoría Taxonómica
        </h2>
