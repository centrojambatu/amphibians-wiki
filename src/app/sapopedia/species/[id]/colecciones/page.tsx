...
export default async function ColeccionesPage({ params, searchParams }: PageProps) {
  ...
  const pagina = searchParamsResolved.pagina
    ? Number.parseInt(searchParamsResolved.pagina, 10)
    : 1;
      ...
      {colecciones.length > 0 ? (
        <div className="mb-8 space-y-4">
          {colecciones.map((coleccion: ColeccionCompleta) => {
            ...
            return (
              <Link key={coleccion.id_coleccion} href={coleccionUrl}>
                <Card className="border transition-colors hover:bg-muted/50 cursor-pointer">
        ...
        <div className="mb-8 rounded-lg border bg-card p-8 text-center">
          <p className="text-muted-foreground">
            No hay colecciones disponibles para esta especie.
          </p>
        </div>
      )}
