...
export default async function getEspeciesNavegacion(): Promise<EspecieNavegacion[]> {
...
export async function getSiguienteEspecie(taxonId: number): Promise<EspecieNavegacion | null> {
...
export async function getAnteriorEspecie(taxonId: number): Promise<EspecieNavegacion | null> {
  const especies = await getEspeciesNavegacion();
  const currentIndex = especies.findIndex((e) => e.id_taxon === taxonId);
