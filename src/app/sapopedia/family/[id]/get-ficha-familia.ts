...
export default async function getFichaFamilia(idFichaFamilia: string) {
  ...
  if (isNumber) {
    ...
    const nombreNormalizado = idFichaFamilia.trim().replaceAll(/\s+/g, " ");
    ...
    if (errorFicha || !fichaData) {
      console.error("❌ Error al obtener ficha_familia por taxon_id:", errorFicha);
  ...
  if (!taxonId || !idFichaFamiliaNum) {
    console.error("❌ No se pudo obtener taxon_id o id_ficha_familia");
  ...
  const [
    {data: fichaFamiliaData, error: errorFichaFamilia},
    ...
    // Obtener todos los campos de ficha_familia
    supabaseClient
      .from("ficha_familia")
      .select("*")
      .eq("id_ficha_familia", idFichaFamiliaNum)
      .eq("taxon_id", taxonId)
      .single(),
    supabaseClient
  ...
  // Manejar errores
  if (errorFichaFamilia) {
    console.error("Error ficha_familia:", errorFichaFamilia);
  }
  ...
  const fichaFamilia = fichaFamiliaData || ({} as any);
  ...
  const camposTexto: (string | null)[] = [
    fichaFamilia?.sinonimia ?? null,
    fichaFamilia?.etimologia ?? null,
    fichaFamilia?.definicion ?? null,
    fichaFamilia?.contenido ?? null,
    fichaFamilia?.distribucion ?? null,
    fichaFamilia?.observaciones ?? null,
    fichaFamilia?.agradecimientos ?? null,
  ];
  ...
  const result = {
    // Campos de ficha_familia
    id_ficha_familia: idFichaFamiliaNum ?? null,
    taxon_id: taxonId,
    sinonimia: fichaFamilia?.sinonimia ?? null,
    etimologia: fichaFamilia?.etimologia ?? null,
    definicion: fichaFamilia?.definicion ?? null,
    contenido: fichaFamilia?.contenido ?? null,
    distribucion: fichaFamilia?.distribucion ?? null,
    observaciones: fichaFamilia?.observaciones ?? null,
    agradecimientos: fichaFamilia?.agradecimientos ?? null,
    // Campos de taxon
  ...
  console.log("📊 Datos retornados ficha_familia:", {
    id_ficha_familia: result.id_ficha_familia,
