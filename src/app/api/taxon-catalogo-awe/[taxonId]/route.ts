...
export async function PUT(
  ...
  try {
    ...
    const { data: registrosActuales, error: errorActuales } =
    ...
    const tipoCatalogoId = Number(tipo_catalogo_awe_id);
    if (Number.isNaN(tipoCatalogoId)) {
      return NextResponse.json(
        { error: "tipo_catalogo_awe_id inválido" },
        { status: 400 },
      );
    }

    const { data: catalogosDelTipo, error: errorCatalogos } =
      await supabaseClient
        .from("catalogo_awe")
    ...
    const idsAAgregar = catalogo_awe_ids.filter(
      (id: number) => !idsActuales.has(id),
    );
    ...
    if (idsAAgregar.length > 0) {
      ...
      const { data: registrosExistentes, error: errorVerificar } =
        await supabaseClient
          .from("taxon_catalogo_awe")
      ...
      const idsParaAgregar = idsAAgregar.filter(
        (id: number) => !idsExistentes.has(id),
      );

      if (idsParaAgregar.length > 0) {
        const nuevosRegistros = idsParaAgregar.map(
